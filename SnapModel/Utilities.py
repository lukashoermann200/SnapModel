...
def runSnapModel(snap_model, StartZ, StopZ, ZStep, osc_dir_0, osc_dir, f0, NSteps):
    ...
    output_array = np.vstack((apex_height_list, theta_list.T, phi_list.T, force.T, U.T, U_PES.T, dissipation))
    
    return output_array


def saveData(tors, CO_length, horizontal_offset, asc_amp, angle, themal_activation,
             temperature, ML, NSteps, StartZ, StopZ, ZStep, output_array, base_dir=None):
    ...
    if ML:
        np.save(base_dir+'/snap_model_ML_TA_tors{:d}_offset{}_amp{}_angle{}_temperature{}'.format(
            int(tors*1e21), horizontal_offset, asc_amp, angle, temperature), output_array)
    else:
        np.save(base_dir+'/snap_model_LJ_TA_tors{:d}_offset{}_amp{}_angle{}_temperature{}'.format(
            int(tors*1e21), horizontal_offset, asc_amp, angle, temperature), output_array)
...
def readData(file_name):
    ...
    return theta[:,z_ind], phi[:,z_ind], force[:,z_ind], U[:,z_ind], U_PES[:,z_ind]


def saveDataAsImage(base_dir, tors, CO_length, horizontal_offset, asc_amp, angle, 
                    temperature, PES_method, NSteps, StartZ, StopZ, ZStep, output_array):
    ...
    for i in range(plot_steps):
        ...
        apex_pos = asc_amp * np.sin( 2 * np.pi * np.linspace(0, 1, len(theta_list[i,:])) )
    ...
    l0 = StopZ - StartZ
    plot_height_corrected = plot_height/plot_steps*(plot_steps-1) + plot_height_individual/plot_steps
    ...
    fig.savefig(base_dir+'/snap_model_'+PES_method+'_TA_tors{:d}_offset{}_amp{}_angle{}_temperature{}.png'.format(
            int(np.round(tors*1e21)), horizontal_offset, asc_amp, angle, temperature), dpi=600)
    
    plt.close()
    
    
def getEnergyDissipationFromFilesTA(output,
                                    PES_method,
                                    tors_filter,
                                    angle_filter,
                                    amp_filter,
    ...
    for file in file_list:
        if PES_method in os.path.basename(file):
            ...
            temperature = re.findall(s+'[0-9\.\-]*', os.path.basename(file))[0]
            if temperature[-1] == '.':
            ...
            for height_ind in range( len(data[-1]) ):
                ...
                if not params in energy_disspation_dict:
                    ...
                    energy_disspation_dict[params] = np.max([energy_disspation_dict[params], energy_dissipation])
...
def getEnergyDissipationFromFiles(output, PES_method, max_offset, amp_plot,
                                  min_ind, max_ind, angle_correction=0.0,
                                  exclude_string='None',
    ...
    file_list = glob.glob(output + '/*.npy')
    for file in file_list:
        if PES_method in os.path.basename(file) and not exclude_string in os.path.basename(file):
            s = '(?<='+re.escape('amp')+')'
            amp = re.findall(s+'[0-9\.\-]*', os.path.basename(file))[0]
            amp = np.float64(amp)
            ...
            if not params in energy_disspation_dict:
                ...
                energy_disspation_dict[params] = np.max([energy_disspation_dict[params], max_energy_dissipation])
