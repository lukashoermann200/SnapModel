...
class EnergyModelML():
    def __init__(self, run, geom, tip, osc_center, osc_dir, asc_amp, CO_length, x_min, x_max, z_min, z_max, energy_offset):
        self.run = run
        self.geom = geom
        self.tip = tip
    ...
    def getInterpolatedEnergy(self):
        x_array_0 = np.linspace(self.x_min, self.x_max, self.interpolation_grid)
        z_array_0 = np.linspace(self.z_min, self.z_max, self.interpolation_grid)
...
class SnapModel():
    ...
    def runOscillation(self, f0, NSteps, apex_height, th, phi):
        ...
        for i in range(self.init_steps):
            for cnt in range(NSteps):
                ...
                apex_pos = self.osc_center + np.array([0.0, 0.0, apex_height]) + vec_x
                
                U, ThNow, PhiNow = self.AdvTip(apex_pos[0], apex_pos[1], apex_pos[2], ThNow, PhiNow)
        ...
        for cnt in range(NSteps):
            ...
            U, ThNow, PhiNow = self.AdvTip(XVec[cnt], Ynow, Znow, ThNow, PhiNow)
            self.UVec[cnt] = U
    ...
    def AdvTip1D(self, Xnow, Ynow, Znow, ThNow, PhiNow):
        ...
        res = scipy.optimize.minimize(func, x0=x0, bounds=limits, tol=1e-7, method=self.optimizer)
