...
class EnergyModelML():
    def __init__(self, run, geom, tip, osc_center, osc_dir, asc_amp,
                 CO_length, x_min, x_max, z_min, z_max, energy_offset,
                 dims=1, interpolation_grid=(40, 10, 40)):
        self.run = run
        self.geom = geom
        self.tip = tip
    ...
    def getInterpolatedEnergy(self):
        x_array_0 = np.linspace(self.x_min, self.x_max, self.interpolation_grid[0])
        z_array_0 = np.linspace(self.z_min, self.z_max, self.interpolation_grid[2])
    ...
    def getInterpolatedEnergy2D(self):
        x_array_0 = np.linspace(self.x_min, self.x_max, self.interpolation_grid[0])
        y_array_0 = np.linspace(self.y_min, self.y_max, self.interpolation_grid[1])
        z_array_0 = np.linspace(self.z_min, self.z_max, self.interpolation_grid[2])
        ...
        for x in x_array_0:
            for y in y_array_0:
                for z in z_array_0:
                    ...
                    tip_pos = self.osc_center + vec_x + vec_y + np.array([0.0, 0.0, z])
    ...
    def getForceOnTip(self, pos):
        ...
        force_osc_dir = (energy_x0 - energy_x1) / (2*np.linalg.norm(d_osc_dir))
        ...
        force = self.osc_dir*force_osc_dir + np.array([0.0, 0.0, 0.01])*force_z
    ...
    def getForceOnTip2D(self, pos):
        ...
        force_osc_dir = (energy_osc_dir_0 - energy_osc_dir_1) / (2*np.linalg.norm(d_osc_dir))
        
        energy_osc_dir_normal_0 = self.getEnergy(pos + d_osc_dir_normal)
        energy_osc_dir_normal_1 = self.getEnergy(pos - d_osc_dir_normal)
        force_osc_dir_normal = (energy_osc_dir_normal_0 - energy_osc_dir_normal_1) \
        ...
        force = self.osc_dir*force_osc_dir + self.osc_dir_normal*force_osc_dir_normal \
...
def getLJParams(species):
    ...
    LJ_eps = np.sqrt(e_probe * e_atom) / 1000 #Careful! This is a non-SI value and saved in meV.
...
class EnergyModelLJ():
    def __init__(self, geom):
        ...
        if geom.isPeriodic():
            self.geom = geom.getPeriodicReplica((1,1,1), explicit_replications=([-1,0,1],[-1,0,1],[0]))
        else:
            self.geom = geom
    ...
    def getEnergy(self, pos):
        ...
        U = self.LJ_eps_vec * ( self.LJ_r_vec**12 / rsq**6 - 2 * self.LJ_r_vec**6 / rsq**3 )
    ...
    def getForce(self, pos):
        ...
        for cnt in range(len(coords)):
            ...
            Fmag = 12*LJ_eps*((LJ_r/r)**12)/r - 12*LJ_eps*((LJ_r/r)**6)/r;
...
class EnergyModelPLJ(EnergyModelML):
    def __init__(self, run, geom, tip, osc_center, osc_dir, asc_amp,
                 CO_length, x_min, x_max, z_min, z_max, energy_offset,
                 lam, interpolation_grid=(40, 10, 40)):
    ...
    def fitModel(self):
        ...
        params_0 = np.hstack((LJ_eps_vec*LJ_r_vec**12,
                              0.0*LJ_r_vec,
                              0.0*LJ_r_vec,
    ...
    def getBase(self, pos):
        ...
        return np.hstack((1.0/rsq**6, 1.0/rsq**5, 1.0/rsq**4, 1.0/rsq**3))
...
class EnergyModelMockup():
    ...
    def getEnergy(self, pos):
        ...
        U_z = self.LJ_eps * ( self.LJ_r**12 / pos[2]**12 - 2 * self.LJ_r**6 / pos[2]**6 )
...
class SnapModel():
    ...
    def getApexPos(self, t_now, T, apex_height):
        ...
        apex_pos = self.osc_center + np.array([0.0, 0.0, apex_height]) + apex_vec
    ...
    def forceSnap(self, t_now, T, apex_height, U, theta_0, phi_0):
        ...
        if self.themal_activation:
            ...
            if apex_phase == self.min_apex_phase and CO_side > 0 \
            or apex_phase == self.max_apex_phase and CO_side < 0:
                theta = -np.pi * 0.1 * CO_side
                phi = self.Phi_init
                U, theta, phi = self.AdvTip(apex_pos, theta, phi)
                
                delta_U = self.calculateBarrier(apex_pos, theta_0, phi_0, theta, phi)
    ...
    def calculateBarrier(self, apex_pos, theta_0, phi_0, theta_1, phi_1):
        ...
        for ind_0, theta in enumerate( theta_list ):
            for ind_1, phi in enumerate( phi_list ):
                U_array[ind_0, ind_1] = self.getEnergyOfTip(apex_pos, theta, phi)
        
        minimum_0 = ( np.argmin(abs(theta_list - theta_0)), np.argmin(abs(phi_list - phi_0)) )
        minimum_1 = ( np.argmin(abs(theta_list - theta_1)), np.argmin(abs(phi_list - phi_1)) )
        
        delta_U = getBarrierHeight(U_array, minimum_0, minimum_1, barrier_to_get=0)[0]
    ...
    def runOscillation(self, f0, NSteps_0, apex_height, th, phi):
        ...
        for i in range(self.init_steps):
            for cnt in range(NSteps):
                ...
                U, ThNow, PhiNow, delta_U = self.forceSnap(t_now, T, apex_height, U, ThNow, PhiNow)
        ...
        for cnt in range(NSteps):
            ...
            Edissint = Edissint - self.FVec[cnt] * np.cos(2*np.pi*f0*cnt*dt)
            
        df = -(f0**2/self.k/self.asc_amp) * dfint * dt
        E_diss = (2*np.pi*self.asc_amp*f0) * Edissint * dt
        
        if self.themal_activation:
            E_diss = self.correctEnergyBySnappingProbability(E_diss, delta_U_vec)
    ...
    def correctEnergyBySnappingProbability(self, E_diss, delta_U_vec):
        p = np.exp( -np.max(delta_U_vec) / (Units.BOLTZMANN_CONSTANT/Units.EV_IN_JOULE) / self.temperature )
    ...
    def AdvTip1D(self, apex_pos, ThNow, PhiNow):
        ...
        res = scipy.optimize.minimize(func, x0=x0, bounds=limits, tol=self.tol, method=self.optimizer)
...
def plotGeometry(geom, osc_center, osc_dir):
    ...
    fig = plt.figure()
    geom.visualize()
    plt.plot([osc_center[0]-0.5*osc_dir[0], osc_center[0]+0.5*osc_dir[0]],
             [osc_center[1]-0.5*osc_dir[1], osc_center[1]+0.5*osc_dir[1]],
             '-', zorder=100000000 )
    plt.plot( [osc_center[0], osc_center[0]], [osc_center[1], osc_center[1]], '.', zorder=100000000 )
